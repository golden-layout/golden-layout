...
export class EventEmitter {
  /** @internal */
  private static _allEventSubscriptions: EventEmitter.UnknownCallback[] = [];
  /** @internal */
  private static _subscriptionsMap = new Map<string, EventEmitter.UnknownCallback[]>();
  ...
  emit<K extends keyof EventEmitter.EventParamsMap>(eventName: K, ...args: EventEmitter.EventParamsMap[K]): void {
    let subcriptions = EventEmitter._subscriptionsMap.get(eventName);
  ...
  emitUnknown(eventName: string, ...args: EventEmitter.UnknownParams): void {
    let subs = EventEmitter._subscriptionsMap.get(eventName);
  ...
  private addUnknownEventListener(eventName: string, callback: EventEmitter.UnknownCallback): void {
    if (eventName === EventEmitter.ALL_EVENT) {
      EventEmitter._allEventSubscriptions.push(callback);
    } else {
      let subscriptions = EventEmitter._subscriptionsMap.get(eventName);
      if (subscriptions !== undefined) {
        subscriptions.push(callback);
      } else {
        subscriptions = [callback];
        EventEmitter._subscriptionsMap.set(eventName, subscriptions);
      }
    }
  }
  ...
  private removeUnknownEventListener(eventName: string, callback: EventEmitter.UnknownCallback): void {
    if (eventName === EventEmitter.ALL_EVENT) {
      this.removeSubscription(eventName, EventEmitter._allEventSubscriptions, callback);
    } else {
      const subscriptions = EventEmitter._subscriptionsMap.get(eventName);
      if (subscriptions === undefined) {
  ...
  private emitAllEvent(eventName: string, args: unknown[]) {
    const allEventSubscriptionsCount = EventEmitter._allEventSubscriptions.length;
    if (allEventSubscriptionsCount > 0) {
      const unknownArgs = args.slice() as EventEmitter.UnknownParams;
      unknownArgs.unshift(eventName);

      const allEventSubcriptions = EventEmitter._allEventSubscriptions.slice();
