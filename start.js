$(() => {
  ...
  const rotate = function(container) {
    if (!container) return;
    while (container.parent && container.type != 'stack') container = container.parent;
    if (container.parent) {
      const p = container.header.position();
      const sides = ['top', 'right', 'bottom', 'left', false];
      const n = sides[(sides.indexOf(p) + 1) % sides.length];
      container.header.position(n);
    }
  };
  const nexttheme = function() {
    let link = $('link[href*=theme]'),
      href = link.attr('href').split('-');
    const themes = ['dark', 'light', 'soda', 'translucent'];
    href[1] = themes[(themes.indexOf(href[1]) + 1) % themes.length];
    link.attr('href', href.join('-'));
  };

  myLayout.registerComponent('html', (container, state) => {
    container
      .getElement()
