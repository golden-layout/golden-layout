...
lm.utils.copy(lm.LayoutManager.prototype, {
  ...
  createDragSource(element, itemConfig) {
    ...
    const dragSource = new lm.controls.DragSource($(element), itemConfig, this);
  ...
  _createConfig(config) {
    const windowConfigKey = lm.utils.getQueryStringParam('gl-window');
...
lm.utils.copy(lm.controls.Header.prototype, {
  ...
  _createControls() {
    ...
    if (this._getHeaderSetting('maximise')) {
      ...
      this.parent.on('maximised', () => {
        maximiseButton.element.attr('title', minimiseLabel);
      });

      this.parent.on('minimised', () => {
...
lm.utils.copy(lm.items.RowOrColumn.prototype, {
  ...
  removeChild(contentItem, keepChild) {
     ...
     */
    const docked = this._isDocked();
    for (i = 0; i < this.contentItems.length; i++) {
      if (this.contentItems[i] !== contentItem) {
        if (!this._isDocked(i))
          this.contentItems[i].config[this._dimension] +=
            removedItemSize / (this.contentItems.length - 1 - docked);
      }
    }
  ...
  _respectMinItemWidth() {
    let minItemWidth = this.layoutManager.config.dimensions
